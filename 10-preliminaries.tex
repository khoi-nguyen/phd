    ...
    \begin{enumerate}
        ...
        \item The set
            \begin{align*}
                ...
                \defeq
                \{ A \in \Lin{\VectorSpace} : A \transpose A = \transpose A A = \Id \VectorSpace \text{ and } \det A = 1 \}
            \end{align*}
...
\begin{example}[Exponential map on $\GeneralLinear n$]
    ...
    \begin{align}
        ...
        A \to \sum_{k = 0}^{+\infty} \frac 1 {k!} A^k.
...
\begin{definition}[Schwartz space]
    We shall say that $f \in \SmoothFunctions \GroupDirect$ is \emph{rapidly decreasing}
    if for every $N \in \N$,
    \begin{align*}
...
\begin{theorem}[Schwartz Kernel Theorem~\cite{Treves67}]
...
Following~\cite[Section 5.1]{FischerRuzhansky16},
we define the convolution between two distributions associated with left-invariant operators that are bounded between appropriate Sobolev spaces.

\begin{definition}[Convolution of distributions]
    Let $\kappa_1, \kappa_2 \in \TemperedDistributions \Group$
    and write
    \begin{align*}
        T_i : \phi \in \Schwartz \Group \mapsto \conv \phi {\kappa_i},
        \quad i = 1, 2.
    \end{align*}
    If there exists $a, b, c \in \R$ such that
    $T_1$ is bounded from $\Sobolev a$ to $\Sobolev b$ and
    $T_2$ is bounded from $\Sobolev b$ to $\Sobolev c$,
    then we define $\conv {\kappa_1} {\kappa_2}$ to be the right convolution kernel of
    \begin{align*}
        \phi \in \Schwartz \Group \mapsto (T_1 \circ T_2) (\phi).
    \end{align*}
\end{definition}
