...
\index{generalised motion group}
    ...
    if there exists a (finite dimensional) vector space $\VectorSpace$ and a compact group $\CompactGroup \subset \SpecialOrthogonalGroup{\VectorSpace}$ such that $\Group = \VectorSpace \rtimes \CompactGroup$ and $\CompactGroup$ acts transitively on $\VectorSpace$.
\end{definition}
