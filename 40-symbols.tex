...
\begin{proof}
    Suppose first that $\sigma \in \SmoothSymbols$ so that its kernel $\kappa_{x, k} \in \Schwartz \Group$.
    By definition, we have
    \begin{align*}
        \DifferenceOperator{q} \LeftDifferentialOperator{X} \sigma(x, k; \lambda)
        = \int_\Group q((y, l)^{-1}) \kappa_{x, k}((y, l)^{-1}) \e^{\i \turn \ip {u^{-1} \lambda} y} \RightRegularRepresentation(l) \dd (y, l)\\
    ...
    We can now substitute $y$ for $y + x$ and $l$ for $k l$ to obtain
    \begin{align*}
        \DifferenceOperator{q} \LeftDifferentialOperator{X} \sigma(x, k; \lambda)
        = \int_\Group q(-l^{-1} y, l^{-1}) \tilde \kappa_{x, k}(-y, l^{-1}) \e^{\i \turn \ip {k u^{-1} \lambda} {(y - x)} \RightRegularRepresentation(l) \dd (y, l).
    \end{align*}
\end{proof}
...
\begin{theorem}
    The operator $\Rotation$ restricts to a map
    \begin{align*}
