...
\begin{lemma}
    Let $Y \in \LieAlgebraCompactGroup$.
    The following expressions hold:
    \begin{align*}
        \LeftDifferentialOperatorFirstOrder Y_l \sigma(x, k; l \lambda)
        = \DifferenceOperator{j} \sigma(x, k; l \lambda) \Rep {l Y \lambda} (X_j).
    \end{align*}
\end{lemma}
\begin{proof}
    ...
    \begin{align}
        \LeftDifferentialOperatorFirstOrder Y_l \sigma(x, k; l \lambda)
        &= \sum_{j = 1}^{\dim \VectorSpace} \D{s}<s = 0> \sigma(x, k; l \lambda + s u e_j) \D{t}<t = 0> \ip {l \exp_\CompactGroup (t Y) \lambda} {u e_j}.
    ...
    \begin{align*}
        ...
        &= \ip {l Y \lambda} {u e_j}\\
        &= \frac{1}{\i \turn} \Rep {l Y \lambda} (X_j),
    \end{align*}
    ...
    while
    \begin{align*}
        \ip {l \exp_\CompactGroup (t Y) \lambda} {e_j}
        &= \ip {l Y \lambda} {e_j}
        = \frac{1}{\i \turn} \Rep[\VectorSpace] {l Y \lambda} {\partial_j}.
    \end{align*}

    Therefore, it follows that \eqref{eq:k_differentiation_of_lambda_variable_in_symbol_2} becomes
    \begin{align}
        \LeftDifferentialOperatorFirstOrder Y_l \tilde \sigma(x, k; l \lambda)
        &= \sum_{j = 1}^{\dim \VectorSpace} \DifferenceOperator[\VectorSpace]{j} \tilde \sigma(x, k; l \lambda) \Rep[\VectorSpace] {l Y \lambda} {\partial_j}.
    \end{align}
\end{proof}
