...
\label{theorem:kernel_estimates}
    ...
    \begin{enumerate}
        \item \label{item:kernel_estimates:at_infinity}
            ...
            there exists $C \geq 0$ such that for every $g, h \in \Group$ with $\norm [\Group] {h} \geq 1$, we have
            \begin{align*}
                \abs{\kappa_g(h)} \leq C \norm [\Group] h^{-N}.
            \end{align*}
        \item \label{item:kernel_estimates:at_origin:positive}
            If $\dim \Group + m > 0$, there exists $C \geq 0$ such that for every $g, h \in \Group$ with $h \neq 0$, we have
            \begin{align*}
        ...
        \item \label{item:kernel_estimates:at_origin:negative}
            If $\dim \Group + m < 0$, $\kappa_g$ is continuous on $\Group$ and is bounded
            \begin{align*}
...
This is, of course, a consequence of the Sobolev inequality.
...
\begin{proof}
    ...
    Since $m + s < \dim \Group / 2$,
    and since $\BesselPotential s \kappa_g$ is the kernel of the symbol $\Rep \lambda \BesselPotential s \sigma(g, \lambda)$ which is of order $m + s$,
    Proposition~\ref{proposition:L2_bound_on_the_kernel} implies that $\BesselPotential s \kappa_g \in \Lebesgue 2 \Group$ for each $g \in \Group$ and
    \begin{align*}
        \sup_{g \in \Group} \norm [\Lebesgue 2 \Group] {\BesselPotential s \kappa_g} < \infty.
    \end{align*}

    Since $s > \dim \Group / 2$,
    and because we have shown that $\kappa_g \in \Sobolev s$,
    we can apply the Sobolev inequality (Proposition~\ref{proposition:Sobolev_embedding}),
    which implies that $\kappa_g$ is continuous and
    \begin{align*}
        \sup_{g, h \in \Group} \abs{\kappa_g(h)}
...
Parts \ref{item:kernel_estimates:at_origin:positive}, \ref{item:kernel_estimates:at_origin:zero} and \ref{item:kernel_estimates:at_origin:negative}.
...
\begin{lemma}
    ...
    \begin{align*}
        ...
        \label{eq:kernels_convoluted_by_Littlewood-Paley_kernels}
    \end{align*}
