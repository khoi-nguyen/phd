...
\index{Littlewood-Paley decomposition}
    Let $A$ be a \emph{locally compact abelian Lie group},
    and suppose that $\CompactGroup \subset \Aut(A)$ is a \emph{compact} group such that the \emph{Haar measure of $A$ is invariant under the action of $\CompactGroup$}.

    If there exists a sequence ${(\chi_j)}_{j \in \N}$ such that
    \begin{enumerate}
        \item they sum to $1$, i.e.
            \begin{align*}
                \sum_{j = 0}^\infty \chi_j = 1
            \end{align*}
        \item for each $j \in \N$, and every $q \in \Polynomials{\VectorSpace}$.
            \begin{align*}
                (\DifferenceOperator{q} \Fourier \chi_j)(k \lambda) = \sum_{r = 1}^{C_q} f_{j, r}(\lambda) q_r(k)
            \end{align*}
        \item condition in $\lambda$?
    \end{enumerate}
