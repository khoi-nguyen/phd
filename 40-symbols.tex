...
\begin{definition}[Rotation of symbols]
    Let $\sigma \in \SymbolClass[\GroupDirect]{m}{\rho, \delta}$.
    We define the operator
    \begin{align*}
        \Rotation \sigma : \SmoothFunctions \CompactGroup \to \SmoothFunctions \CompactGroup
    \end{align*}
    via the formula
    \begin{align*}
        \Rotation \sigma(x, k; \lambda) F(u) \defeq \sigma(x, k; k u^{-1} \lambda) F(u),
    \end{align*}
    where $x, \lambda \in \VectorSpace$, $k, u \in \CompactGroup$, and $F \in \SmoothFunctions \CompactGroup$.
\end{definition}

\begin{lemma}
    Let $q \in \SmoothFunctions{\Group}$ and $X \in \LieAlgebra$.
    If we define $\tilde q(y, l) = q(l y, l)$,
    then we have
    \begin{align*}
        \sigma(x, k; \lambda) &= \Rotation {\tilde \sigma}(x, k; \lambda)\\
        \DifferenceOperator{q} \LeftDifferentialOperatorFirstOrder{X} \sigma(x, k; \lambda)
        &= \Rotation {
            \DifferenceOperator[\GroupDirect] {\tilde q} \LeftDifferentialOperatorFirstOrder{X} \tilde \sigma
            + \sum_{j = 1}^{\dim \VectorSpace} (\DifferenceOperator[\GroupDirect] {\tilde q} \DifferenceOperator[\VectorSpace]{j} \tilde \sigma) \Rep[\GroupDirect] {k \Projection \LieAlgebraCompactGroup X k^{-1} \lambda} (\partial_j)
        }(x, k; \lambda)
    \end{align*}
\end{lemma}
\begin{proof}
    ...
    Using $(y, l)^{-1} (x, k) = (l^{-1}(x - y), l^{-1} k)$ and
    \begin{align*}
        \kappa_{x, k} ((y, l)^{-1} (x, k)) = \tilde \kappa_{x, k} (y - x, l^{-1} k),
    \end{align*}
    ...
    \end{align*}
    Now, applying $\LeftDifferentialOperatorFirstOrder{X}$ on both sides of the above,
    we get
    \begin{align*}
        \DifferenceOperator{q} \LeftDifferentialOperatorFirstOrder{X} \sigma(x, k; \lambda)
        &= \DifferenceOperator[\GroupDirect] {\tilde q} \eval{\LeftDifferentialOperatorFirstOrder{X}}{(x', k') = (x, k)} \tilde \sigma(x', k', k u^{-1} \lambda) F(u)\\
        &\quad+ \sum_{j = 1}^{\dim \VectorSpace} \DifferenceOperator[\GroupDirect] {\tilde q} \D[\tilde \sigma]{\lambda_j}(x, k; k u^{-1} \lambda) \LeftDifferentialOperatorFirstOrder{X}_k \ip {k u^{-1} \lambda} {e_j}\\
        &= \Rotation {\DifferenceOperator[\GroupDirect] {\tilde q} \LeftDifferentialOperatorFirstOrder{X} \tilde \sigma}(x, k, \lambda) F(u)\\
        &\quad+ \sum_{j = 1}^{\dim \VectorSpace} \Rotation {\DifferenceOperator[\GroupDirect] {\tilde q} \D[\tilde \sigma]{\lambda_j}} (x, k; \lambda) \LeftDifferentialOperatorFirstOrder{X}_k \ip {k u^{-1} \lambda} {e_j}
    \end{align*}

    Computing
    \begin{align*}
        \LeftDifferentialOperatorFirstOrder{X}_k \ip {k u^{-1} \lambda} {e_j}
        &= \D*{t}<t = 0> \ip {k \exp_\CompactGroup(t \Projection \LieAlgebraCompactGroup X) u^{-1} \lambda} {e_j}\\
        &= \ip {k \Projection \LieAlgebraCompactGroup X u^{-1} \lambda} {e_j}\\
        &= \frac 1 {\i \turn} \Rep[\GroupDirect] {k \Projection \LieAlgebraCompactGroup X k^{-1} k u^{-1} \lambda} (\partial_j)\\
        &= \frac 1 {\i \turn} \Rotation {\Rep[\GroupDirect] {k \Projection \LieAlgebraCompactGroup X k^{-1} \lambda} (\partial_j)},
    \end{align*}
    we obtain
    \begin{align*}
        \DifferenceOperator{q} \LeftDifferentialOperatorFirstOrder{X} \sigma(x, k; \lambda)
...
\begin{theorem}
    The operator $\Rotation \dummy$ restricts to a map
    \begin{align*}
        \Rotation : \SymbolClass[\GroupDirect]{m}{\rho, \delta} \to \SymbolClass{m}{\rho, \delta}.
    \end{align*}
    Moreover, we have
    \begin{align*}
        \Op[\GroupDirect](\sigma) = \Op(\Rotation \sigma),
    \end{align*}
    which implies that $\OperatorClass[\GroupDirect] m {\rho, \delta} \subset \OperatorClass m {\rho, \delta}$.
\end{theorem}
\begin{proof}
    Let $\sigma \in \SymbolClass[\GroupDirect] m {\rho, \delta}$.
    \begin{description}
        \item[Step 1:] $\Op[\GroupDirect](\sigma) = \Op(\Rotation \sigma)$.

            Let $\phi \in \Schwartz \Group$, $(x, k) \in \Group$ and write $T = \Op[\GroupDirect](\sigma)$.
            If $e_i$, $i \in \N$ is an orthonormal basis of $\Lebesgue 2 \CompactGroup$, then
            \begin{align*}
                T \phi(x, k)
                &=
                \int_\VectorSpace
                    \sum_{i \in \N}
                    \ip[\Lebesgue 2 \CompactGroup]{%
                        \sigma(x, k; \lambda) e_i
                    }{%
                        \adj{\left(\Fourier[\GroupDirect] \phi(\lambda) \Rep[\GroupDirect] \lambda(x, k)\right)} e_i
                    }
                \dd \lambda\\
                &=
                \int_\VectorSpace
                    \sum_{i \in \N}
                    \int_\CompactGroup
                        \sigma(x, k; \lambda) e_i(u)
                        \conj{\adj{\left(\Fourier[\GroupDirect] \phi(\lambda) \Rep[\GroupDirect] \lambda(x, k)\right)} e_i(u)}
                    \dd u
                \dd \lambda.
            \end{align*}

            Applying the Fubini Theorem\todo{Justify interchange integrals-sum},
            we can interchange the integrals and the sum freely,
            then substitute $\lambda$ for $k u^{-1} \lambda$ to obtain
            \begin{align}
                T \phi(x, k)
                =&
                \int_\VectorSpace
                    \sum_{i \in \N}
                    \int_\CompactGroup
                        \Rotation \sigma(x, k; \lambda) e_i(u) \notag\\
                        &\quad \conj{\adj{\left(\Fourier[\GroupDirect] \phi(k u^{-1} \lambda) \Rep[\GroupDirect] {k u^{-1} \lambda}(x, k)\right)} e_i(u)}
                    \dd u
                \dd \lambda.
                \label{eq:rotation_of_direct_quantisation}
            \end{align}

            Now, we observe that
            \begin{align*}
                &\adj{\left(\Fourier[\GroupDirect] \phi(k u^{-1} \lambda) \Rep[\GroupDirect] {k u^{-1} \lambda} (x, k)\right)} e_i(u)\\
                &=
                \int_\VectorSpace
                    \int_\CompactGroup
                        \conj{\phi(y, l)}
                        \e^{\i \turn \ip {u^{-1} \lambda} {k^{-1} (y - x)}} \RightRegularRepresentation(k^{-1} l) e_i(u)
                    \dd l
                \dd y\\
                &=
                \int_\VectorSpace
                    \int_\CompactGroup
                        \conj{\phi(y, l)}
                        \Rep \lambda (k^{-1}(y - x), k^{-1} l) e_i(u)
                    \dd l
                \dd y.
            \end{align*}
            Observing that
            \begin{align*}
                (k^{-1}(y - x), k^{-1} l) = {(x, k)}^{-1} (y, l) = {\left({(y, l)}^{-1} (x, k)\right)}^{-1},
            \end{align*}
            it follows that
            \begin{align*}
                &\adj{\left(\Fourier[\GroupDirect] \phi(k u^{-1} \lambda) \Rep[\GroupDirect] {k u^{-1} \lambda} (x, k)\right)} e_i(u)\\
                ...
                \int_\VectorSpace
                    \int_\CompactGroup
                        \conj{\phi(y, l)}
                        \adj{\Rep \lambda \left((y, l)^{-1} (x, k)\right)} e_i(u)
                    \dd l
                \dd y\\
                &\quad= \adj{\left(\Fourier \phi(\lambda) \Rep \lambda (x, k) \right)} e_i(u)
            \end{align*}

            Using the above in~\eqref{eq:rotation_of_direct_quantisation},
            we obtain
            \begin{align*}
                T \phi(x)
                &=
                \int_\VectorSpace
                    ...
                    \int_\CompactGroup
                        \Rotation \sigma(x, k; \lambda) e_i(u)
                        \conj{\adj{\left(\Fourier \phi(\lambda) \Rep \lambda (x, k) \right)} e_i(u)}
                    \dd u
                \dd \lambda\\
                &=
                \int_\VectorSpace
                    \sum_{i \in \N}
                        \ip{%
                            \Rotation \sigma(x, k; \lambda) e_i
                        }{%
                            \adj{\left(\Fourier \phi(\lambda) \Rep \lambda (x, k) \right)} e_i
                        }
                \dd \lambda\\
                &=
                \int_\VectorSpace
                    \tr\left(
                            \Fourier \phi(\lambda) \Rep \lambda (x, k) 
                            \Rotation \sigma(x, k; \lambda)
                    \right)
                \dd \lambda,
            \end{align*}
            which means that $T = \Op(\Rotation \sigma)$.
    \end{description}
\end{proof}
