...
\begin{remark}[Kernel estimates when $m = -\dim \Group$]
    ...
    Unfortunately,
    $\kappa_g - \kappa_{\epsilon, g}$ cannot be estimated by $\log \epsilon$
    as the Schatten norm
    \begin{align*}
        \norm [\SchattenClasses 1 {\Lebesgue 2 \CompactGroup}] {(\Id {\Lebesgue 2 \CompactGroup} - \eta_\epsilon(\lambda)) \Rep \lambda \BesselPotential m}
    \end{align*}
    is not integrable.
    This is due to the fact that $\Id {\Lebesgue 2 \CompactGroup} - \eta_\epsilon$ does not cut off the high frequencies of the Laplacian unlike $\eta_\epsilon$.

    % TODO: Research
    Can it be be possible that it is uniformly bounded in $\epsilon$?
\end{remark}
