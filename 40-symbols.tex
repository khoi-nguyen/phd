...
\end{align}
This allows us to carry out our computations on the resolvent.
...
\begin{proof}
    ...
    \end{align*}
    and observe that $\abs {G_\epsilon(z)} \leq C \abs {\epsilon z}^{-m/4}$ on $\Gamma$ where $C$ does not depend on $\epsilon \in (0, 1]$,
    because
    \begin{align*}
...
\begin{proof}
    ...
    we obtain that~\eqref{eq:composition_formula:bound_on_the_remainder} is bounded by
    \begin{align*}
        \begin{cases}
