...
\begin{remark}[Right semi-norms]
    We leave to the reader to check that in all the estimates of Theorem~\ref{theorem:kernel_estimates}}
    it is possible to replace the symbol semi-norms by $\SymbolSemiNorm {m, R} {\rho, \delta} {P'} \sigma$.
\end{remark}
...
\label{proposition:L2_bound_on_the_kernel}
    ...
    \begin{align*}
        \sup_{g \in \Group} \norm [\Lebesgue 2 \Group] {\kappa_g} \leq C \SymbolSemiNorm m {\rho, \delta} P \sigma < \infty.
    \end{align*}
\end{proposition}
\begin{proof}
