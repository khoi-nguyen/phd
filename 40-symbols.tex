...
\label{theorem:generalised_Littlewood-Paley_decomposition}
    ...
    \begin{enumerate}
        \item For each $m \in \R$ and each $N \in \N$,
            ...
            \end{align*}
        \item For each $m \geq 0$ and each $N \in \N$,
            there exists $C_{m, N} \geq 0$ such that for every $\epsilon \in (0, 1]$,
            we have
            \begin{align*}
...
\begin{proof}
    ...
    \begin{description}
        ...
        \item [Step 2] Computing $\DifferenceOperatorOrder \alpha \eta_\epsilon$.
            ...
            we used the fact that $\InverseFourier [\R^n] \chi_{\epsilon \JapaneseBracket \CompactGroup \tau}$ is invariant under rotations.
        ...
        \item [Step 4a] Conclude when $m \leq 0$.
            ...
            \begin{align*}
                \bigoplus_{\JapaneseBracket \CompactGroup \tau \leq C_\alpha \epsilon^{-1}} V_\tau
            \end{align*}
            for some $C_\alpha$ depending only on $\alpha$.
            As a result,
            we can define the operator
            \begin{align*}
                L(\lambda) \defeq 1_{\{\norm \lambda \leq C \epsilon^{-1}\}} \bigoplus_{\JapaneseBracket \CompactGroup \tau \leq C_\alpha \epsilon^{-1}} \Rep \lambda \BesselPotential 1
            \end{align*}
            and be certain that $L(\lambda)$ and $\Rep \lambda \BesselPotential 1$ coincide on the range of~\eqref{eq:Littlewood-Paley:central_operator}
            for every $\lambda \in \VectorSpace$.

            Moreover,
            it is clear that from the way we constructed $L(\lambda)$ that $\norm [\Lin {\Lebesgue 2 \CompactGroup}] {L(\lambda)} \leq C \epsilon^{-1}$.
            Therefore,
            \begin{align*}
                &\norm [\Lin {\Lebesgue 2 \CompactGroup}] {%
            ...
            \end{align*}
            which concludes the proof when $m \leq 0$.

        \item [Step 4b] Assume now that $m \geq 0$.

            We would like to proceed like in Step 4a,
            where we construct an operator $L(\lambda)$ which is equal to $\Rep \lambda \BesselPotential 1$ on the range of~\eqref{eq:Littlewood-Paley:central_operator},
            but as $m \geq 0$,
            $L(\lambda)^{-m}$ is bounded provided that we can bound $\norm [\Lin {\Lebesgue 2 \CompactGroup}] {L(\lambda)}$ from \emph{below}.

            This is a bit tricky since as $\eval{\eta_\epsilon(\lambda)} {V_\tau}$ is non-zero when $\epsilon$ is sufficiently small,
            even for small $\norm \lambda$ and small $\JapaneseBracket \CompactGroup \tau$.
            We shall therefore decompose $\eta_\epsilon$ into a countable sum of similar symbols.
            ...
            \begin{align*}
                ...
                \end{cases}
            \end{align*}
            so that $\eta_\epsilon = \sum_{j = 0}^{+\infty} \eta_{\epsilon, j}$.
            ...
            Define the operator
            \begin{align*}
                L_j(\lambda) \defeq \bigoplus_{\JapaneseBracket \CompactGroup \tau \leq C \epsilon^{-1}} 1_{\{c \leq 2^{-j} \epsilon \JapaneseBracket \CompactGroup \tau \norm \lambda \leq C\}} \Rep \lambda \BesselPotential 1
            \end{align*}
            ...
            \begin{align*}
                ...
                &\leq C_{\alpha, \gamma, m} \epsilon^{m} \sum_{j = 0}^{+\infty} 2^{-j m}
                = C_{\alpha, \gamma, m} \epsilon^m.
            \end{align*}
    \end{description}

    Now, let us turn our attention to the second claim,
    and fix $m > 0$.
\end{proof}
