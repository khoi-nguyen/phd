...
\label{lemma:prepare_kernel_estimates_at_the_origin}
    ...
    where $1 \geq \rho > \delta \geq 0$.
    For each $g \in \Group$ and each $\epsilon \in (0, 1]$,
    we let
    \begin{align*}
        \kappa_{\epsilon, g} \defeq \InverseFourier \{ \sigma_\epsilon(g, \dummy) \}, \quad \sigma_\epsilon = \sigma(g, \lambda) \eta_\epsilon(\lambda)
    \end{align*}
    where $\eta_\epsilon$ is the symbol defined in Theorem~\ref{theorem:generalised_Littlewood-Paley_decomposition}.

    Fix $h \in \Group$ such that $0 < \norm [\Group] h \leq 1$ and
    suppose that $m_1 < \dim \Group$ and $r \in 2 \N$.

    The following estimates hold.
    \begin{enumerate}
        \item \label{item:prepare_kernel_estimates_at_the_origin:k_epsilon}
            If $m_1 - m + \rho r \leq 0$,
            then
            \begin{align*}
                \abs{\kappa_{\norm [\Group] h, g}(h)} \leq C \norm [\Group] h^{m_1 - m + (\rho - 1) r}.
            \end{align*}
        \item \label{item:prepare_kernel_estimates_at_the_origin:remainder}
            If $m_1 - m + \rho r \geq 0$,
            then
            \begin{align*}
                \abs{\kappa_g(h) - \kappa_{\norm [\Group] h, g}(h)} \leq C \norm [\Group] h^{m_1 - m + (\rho - 1) r}.
            \end{align*}
    \end{enumerate}
\end{lemma}
\begin{proof}
    ...
    \begin{enumerate}
        \item
            ...
            \begin{align*}
                \norm [\Group] h^r
                \abs{\kappa_{\norm [\Group] h, g}(h)}
                &\leq C \sum_{\abs \alpha = r}
                \sup_{\lambda \in \VectorSpace} \SymbolSemiNorm {m} {\rho, \delta} 0 {\DifferenceOperatorOrder \alpha \sigma_\epsilon(g, \lambda)}\\
                &\leq C
                \SymbolSemiNorm{m_1 - m + \rho r} {\rho, \delta} {r} {\eta_{\norm [\Group] h}}.
            \end{align*}

            Since $m_1 - m + \rho r \leq 0$,
            we can apply Theorem~\ref{theorem:generalised_Littlewood-Paley_decomposition} to get
            \begin{align*}
                \norm [\Group] h^r
                \abs{\kappa_{\norm [\Group] h, g}(h)}
                \leq C
                \norm [\Group] h^{m_1 - m + \rho r}.
            \end{align*}
        ...
        \item
            ...
            \begin{align*}
                &\norm [\Group] h^r
                \abs{\kappa_g(h) - \kappa_{\norm [\Group] h, g}(h)}\\
                &\quad \leq C \sum_{\abs \alpha = r}
                \sup_{\lambda \in \VectorSpace} \SymbolSemiNorm {m} {\rho, \delta} 0 {\DifferenceOperatorOrder \alpha (\Id {\Lebesgue 2 \CompactGroup} - \sigma_\epsilon)(g, \lambda)}\\
                &\quad \leq C
                \SymbolSemiNorm{m_1 - m + \rho r} {\rho, \delta} {r} {\Id {\Lebesgue 2 \CompactGroup} - \eta_{\norm [\Group] h}}.
            \end{align*}
            ...
            \begin{align*}
                \norm [\Group] h^r
                \abs{\kappa_{\norm [\Group] h, g}(h)}
                \leq C \norm [\Group] h^{m_1 - m + 2 \rho r}.
            \end{align*}
            This can be rewritten
            \begin{align*}
                \abs{\kappa_{\norm [\Group] h, g}(h)}
                \leq C \norm [\Group] h^{m_1 - m + (2 \rho - 1) r},
            \end{align*}
            concluding the proof.
    \end{enumerate}
\end{proof}
...
\begin{proof}
    ...
    \begin{description}
        \item[Case $m > -\dim \Group$].
            Letting
            \begin{align*}
                m_1 = m \left(1 - \frac 1 \rho\right) - \frac {\dim \Group} \rho > - \dim \Group
            \end{align*}
            so that $m_1 - m = -(\dim \Group + m)/\rho < 0$.
            Applying Part~\ref{item:prepare_kernel_estimates_at_the_origin:k_epsilon} of
            Lemma~\ref{lemma:prepare_kernel_estimates_at_the_origin},
            we get
            \begin{align}
                \abs{\kappa_\epsilon(h)} \leq C \norm [\Group] h^{- \frac {\dim \Group + m} \rho}.
                \label{eq:kernel_estimate_for_large_m:k_epsilon}
            \end{align}
            ...
            \begin{align*}
                ...
                m_1 &\defeq m + r(1 - 2 \rho) - \frac{\dim \Group + m} \rho.
            \end{align*}

            Let us prove that they satisfy the following properties:
            \begin{align}
                \begin{cases}
                    m_1 < -\dim \Group,\\
                    m_1 - m + \rho r \geq 0,\\
                    m_1 - m + (\rho - 1)r = -\frac{\dim \Group + m} \rho.
                \end{cases}
            ...
            \begin{align*}
                m_1 - m + \rho r
                &= r - \frac{\dim \Group + m} \rho \geq 0\\
            \end{align*}
            by definition of $r$.
            Similarly,
            \begin{align*}
                m_1 - m
                &= -r \rho(1 - \frac 1 \rho) - \frac{\dim \Group + m} \rho\\
            ...
            Therefore,
            applying Lemma~\ref{lemma:prepare_kernel_estimates_at_the_origin} yields
            \begin{align}
                \abs{(\kappa_g - \kappa_{\epsilon, g})(h)}
                \leq C \norm [\Group] h^{-\frac{\dim \Group + m} \rho},
                \label{eq:kernel_estimate_for_large_m:remainder}.
            \end{align}

            Combining~\eqref{eq:kernel_estimate_for_large_m:k_epsilon}
            and~\eqref{eq:kernel_estimate_for_large_m:remainder},
            we obtain
            \begin{align*}
                \abs {\kappa_g(h)} \leq C \norm [\Group] h^{-\frac {\dim \Group + m} \rho},
            \end{align*}
            concluding the case $m > -\dim \Group$.
        \item[Case 2: $m = -\dim \Group$.]
            Set $\epsilon \defeq \norm [\Group] h$.

            We observe that
            \begin{align*}
                \sigma_\epsilon(g, \lambda) &= \sigma(g, \lambda) \eta_\epsilon(\lambda)\\
                &= \sigma(g, \lambda) \Rep \lambda \BesselPotential {\dim \Group} \eta_\epsilon(\lambda) \Rep \lambda \BesselPotential {-\dim \Group}.
            \end{align*}

            It follows then that
            \begin{align*}
                \norm [\SchattenClasses 1 {\Lebesgue 2 \CompactGroup}] {\sigma_\epsilon(g, \lambda)}
                &\leq C \norm [\SchattenClasses 1 {\Lebesgue 2 \CompactGroup}] {\eta_\epsilon(\lambda) \Rep \lambda \BesselPotential {-\dim \Group}}\\
                &= C \norm [\SchattenClasses 2 {\Lebesgue 2 \CompactGroup}] {\sqrt{\eta_\epsilon}(\lambda) \Rep \lambda \BesselPotential {-\dim \Group / 2}}\\
            \end{align*}

            Integrating the above with respect to $\lambda$
            and using the inverse Formula yields
            \begin{align*}
                ...
                &\leq C \norm [\LebesgueDual 2 \Group] {\sqrt{\eta_\epsilon} \Rep \lambda \BesselPotential {\dim \Group / 2}}\\
                &\leq C \log \epsilon = C \log \norm [\Group] h,
            \end{align*}
            where the last line was obtained by the Sobolev inequality.

            To estimate $\abs {\kappa_g(h) - \kappa_{\epsilon, g}(h)}$,
            we use the exact same argument as for the case $m > -\dim \Group$.
    \end{description}
\end{proof}
