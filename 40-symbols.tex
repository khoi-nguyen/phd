...
\begin{proof}
    ...
    we know that each $Y^\beta_v \sigma$ belongs to $\SymbolClass 0 {1, 0}$ so that the above becomes
    \begin{align*}
        \sup_{(x, k) \in \Group} \esssup_{\lambda \in \VectorSpace}
        \int_\CompactGroup \sup_{v \in \CompactGroup} \abs {\sigma(x, k; v \lambda) F(u)}^2 \dd u
...
\begin{theorem}
    ...
    In particular, if $\delta \geq 1 - \rho$, the symbol classes coincide (i.e. $\SymbolClass m {\rho, \delta} = \SymbolClass [\GroupDirect] m {\rho, \delta}$).
    Moreover, if we impose the condition $\rho \geq \delta$,
    then we have
    \begin{align*}
        \SymbolClass m {\rho, \delta} = \SymbolClass [\GroupDirect] m {\rho, \delta}
        \quad \text{if } \rho \geq \frac 1 2.
    \end{align*}
\end{theorem}
