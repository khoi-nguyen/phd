...
The aim of this section is to show a result which will be essential hereafter.
...
\label{theorem:functional_calculus}
    ...
    If $F$ is analytic in $\C \setminus \{z \in \C : d_\C(z, \Lambda) \leq \epsilon\}$ for some $\epsilon > 0$
    and satisfies
    \begin{align*}
        \abs {F(z)} \leq C \abs z^s
    \end{align*}
    for some $s < 0$.
    ...
    \begin{align*}
        F(\sigma)(g, \lambda)
        = \frac 1 {\i \turn} \oint_{\partial \Lambda} F(z) (\sigma(g, \lambda) - z \Id {\Lebesgue 2 \CompactGroup})^{-1} \dd z
    \end{align*}
...
\begin{corollary}
    ...
    \end{align*}
    defines a symbol in $\SymbolClass \gamma {\rho, \delta}$.
\end{corollary}
...
\begin{theorem}
    ...
    Moreover, for each $m \in \R$ and each $N \in \N$,
...
\begin{proof}
    ...
    \begin{align*}
        \abs {F_n(z)} \leq C \abs z^s,
    \end{align*}
    Theorem~\ref{theorem:functional_calculus} implies that
    \begin{align*}
        \SymbolSemiNorm {2 s} {\rho, \delta} N {F_n(t \Rep \lambda (\Laplacian))}
        \leq C_{N, n, s} t^s.
    \end{align*}
    ...
    it follows that
    \begin{align*}
        \e^{t \Rep \lambda (\Laplacian)}
        = F_n(t \Rep \lambda (\Laplacian)) (t \Rep \lambda \Laplacian)^n,
    \end{align*}
    which implies in turn that
    \begin{align*}
        \SymbolSemiNorm {2 (s + n)} {\rho, \delta} N {e^{t \Rep \lambda (\Laplacian)}}
        \leq C_{N, n, s} t^{s + n}.
    \end{align*}

    The first estimate follows, since $s < 0$ and $n \in \N$ are arbitrary.

    Now let $m \geq 0$ and let us check the second symbolic estimate.
    To this end,
    we let
    \begin{align*}
        G(z) \defeq z^{-\frac m 2} (1 - \e^z)
    \end{align*}
    and observe that $\abs {G(z)} \leq z^{-m/4}$.
    Arguing like before,
    we obtain
    \begin{align*}
        \SymbolSemiNorm {-m/2} {\rho, \delta} N {G(t \Rep \lambda (\Laplacian))}
        \leq t^{-m/4}
    \end{align*}

    We can now conclude by Corollary
    \begin{align*}
