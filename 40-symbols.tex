...
\begin{proof}
    ...
    \begin{align*}
        ...
        &= \sum_{\abs {\beta_1} + \abs {\beta_2} \leq 2M} \int_\Group \TaylorRemainder {\RightDifferentialOperator [g_2 = h] {\beta_2} \LeftDifferentialOperator [g_1] {\beta_1} \LeftDifferentialOperator {\beta_0} \kappa_{g g_1}^{(*)}(g_2)} {g_1 = e_\Group} {N - \abs {\beta_1}} (h^{-1}) \adj{\Rep \lambda (h)} \dd h.
    \end{align*}
    ...
    \begin{align*}
        ...
        \norm [\Lin {\Lebesgue 2 \CompactGroup}] {%
            \TaylorRemainder {\RightDifferentialOperator [g_2 = h] {\beta_2} \LeftDifferentialOperator [g_1] {\beta_1} \LeftDifferentialOperator {\beta_0} \kappa_{g g_1}^{(*)}(g_2)} {g_1 = e_\Group} {N - \abs {\beta_1}} (h^{-1})
        }
        \dd h.
    \end{align*}
    ...
    \begin{align*}
        ...
        \sup_{\norm [\Group] z \leq \eta \norm [\Group] h}
        \abs {%
            \LeftDifferentialOperator [h] \gamma
            \RightDifferentialOperator [h_2 = g] {\beta_2}
            \LeftDifferentialOperator [g_1] {\beta_1}
            \LeftDifferentialOperator [g] {\beta_0}
            \kappa^*_{g g_1}(h_2)
        }
    \end{align*}
    where the last line was obtained by passing from left to right-invariant derivatives with Corollary~\ref{corollary:from_right_to_left-invariant_differential_operators}.
\end{proof}
