...
\index{Littlewood-Paley decomposition}
    ...
    \begin{enumerate}
        ...
        \item for each $q \in \Polynomials\AbelianGroup$,
            there exists a finite family $q_1, \dots, q_{C_q} \in \Polynomials\AbelianGroup$ such that for each $j \in \N$, we have
            \begin{align*}
                (\DifferenceOperator[\AbelianGroup]{q} \Fourier[\AbelianGroup] \chi_j)(k \lambda) = \sum_{r = 1}^{C_q} f_{j, r}(\lambda) q_r(k),
            \end{align*}
            where the functions $f_j$ are defined on $\dualGroup\AbelianGroup$ such that
            \begin{align*}
                \sup_{r = 1, \dots, C_q} \sup_{j \in \N} \sup_\dualGroup\AbelianGroup \abs{f_{j, r}} < \infty.
            \end{align*}
