...
hence it is important to consider a family with respect to which we can use a \emph{Taylor development}.
...
This, amongst other things, ensures that our collection of symbols forms an algebra with respect to pointwise composition.
...
Therefore,
we shall often attempt to obtain results up to symbols which belong to symbolic classes of ``arbitrarily low orders''.
...
Such kernels can be defined directly in terms of the symbols,
...
To show that the composition of two pseudo-differential operators is well-defined,
we shall need the following result.
Its proof is a simple adaptation of that of~\cite[Theorem~5.2.15]{FischerRuzhansky16}.

\begin{proposition}
    Let $m \in \R$ and assume that $1 \geq \rho \geq \delta \geq 0$.
    If $T \in \OperatorClass{m}$ and $\phi \in \Schwartz \Group$,
    then $T \phi \in \Schwartz \Group$.
    Moreover, $T$ acts \emph{continuously} on $\Schwartz \Group$.
\end{proposition}
...
Secondly, we would like to show that $\SmoothingSymbols$ is dense in $\SymbolClass m {\rho, \delta}$ in a suitable sense (see~Proposition~\ref{proposition:approximation_of_symbols}),
...
\label{lemma:fall_in_order_for_parameter-elliptic_symbols}
    ...
    If $m = 0$,
    the resolvent $(\sigma - z \Id {\Lebesgue 2 \CompactGroup})^{-1}$ is a symbol in $\SymbolClass 0 {\rho, \delta}$.
    Moreover, we have the symbolic estimate
    \begin{align*}
        \SymbolSemiNorm 0 {\rho, \delta} N {(\sigma - z \Id {\Lebesgue 2 \CompactGroup})^{-1}} \leq C_N (1 + \abs z)^{-1}.
    \end{align*}
...
\begin{proof}
    ...
    if the symbol does not depend on $g$.
...
\begin{proof}
    ...
    Theorem~\ref{theorem:functional_calculus}, with~\eqref{eq:functional_calculus:bound_on_seminorm}, implies that
    \begin{align*}
...
These estimates are crucial in the rest of our analysis.
...
\label{remark:right_semi-norms}
    By inspecting the proof of all estimates in Theorem~\ref{theorem:kernel_estimates},
...
\label{corollary:Sobolev_estimates_on_the_kernel}
    ...
    Moreover, for such $s \in \R$, there exists $C \geq 0$ and $P \in \N$ which do not depend on $\sigma$ such that
    \begin{align*}
...
As applying difference operators strictly \emph{reduces the order} when $\rho > 0$,
the distribution
\begin{align*}
...
The \emph{strong admissibility} means that polynomials can constitute a good approximation of the distance to the origin.
...
\begin{proof}
    ...
    Note that the case $m = -\dim \Group$ is a simple consequence of the other estimates,
    ...
    \begin{description}
        \item[Estimates on $\kappa_\epsilon$]
            ...
            \end{align*}
            since $\eta_\epsilon$ is diagonal.
        ...
        \item[Estimate on $\kappa_g - \kappa_{g, \epsilon}$].
            ...
            Observe that these two conditions can be summarized by the inequality
            \begin{align*}
...
For a proof, see e.g.~\cite[Lemma 3.1.55]{FischerRuzhansky16},
...
Before we start,
let us state a result that will be used in the proof of both results.
...
\begin{proof}
    ...
    \begin{description}
        ...
        \item [Integral on] $\norm [\Group] h \leq 1$
            ...
            then Part~\eqref{item:kernel_estimates:at_origin:positive} of Theorem~\ref{theorem:kernel_estimates} implies
...
\label{lemma:prepare_composition_formula}
    ...
    \end{align}
    then there exists $C \geq 0$ and $P \in \N$ such that
...
\begin{proof}
    ...
    which will allow us to estimate the above in terms of the symbol semi-norms of $\sigma_2$.
    To this end,
    ...
    (see Corollary~\ref{corollary:from_right_to_left-invariant_differential_operators}),
    \begin{align}
        &S(h, M, \gamma, \beta_{02}, \beta_2)
...
As this involves only one symbol,
...
\end{theorem}
\begin{proof}
    Let $N \in \N$ and set $m \defeq m_1 + m_2$.
    We write
    \begin{align*}
        \tau_N &\defeq
        \sigma_1 \circ \sigma_2
        - \sum_{\abs \alpha \leq N} \frac 1 {\alpha!} \DifferenceOperatorOrder \alpha \sigma_1 \ \TaylorLeftDifferentialOperator \alpha \sigma_2.
    \end{align*}
    For each $\epsilon \in (0, 1]$,
    we let
    \begin{align*}
        \sigma_{1, \epsilon} &\defeq \eta_\epsilon \sigma_1\\
        \tau_{N, \epsilon} &\defeq
        \sigma_{1, \epsilon} \circ \sigma_2
        - \sum_{\abs \alpha \leq N} \frac 1 {\alpha!} \DifferenceOperatorOrder \alpha \sigma_{1, \epsilon} \ \TaylorLeftDifferentialOperator \alpha \sigma_2.
    \end{align*}

    We check that
    \begin{align*}
        \tau_{N, \epsilon} = \eta_\epsilon \tau_N - L_\epsilon,
    \end{align*}
    where
    \begin{align*}
        L_\epsilon \defeq
        \sum_{\abs \alpha \leq N}
        \sum_{\substack{\abs \alpha \leq \abs {\alpha_1} \eta_\epsilon + \abs {\alpha_2} \leq 2 \abs \alpha \\ \alpha_1 \neq 0}}
        \frac 1 {\alpha!} \DifferenceOperatorOrder {\alpha_1} \DifferenceOperatorOrder {\alpha_2} \sigma_1 \ \TaylorLeftDifferentialOperator \alpha \sigma_2.
    \end{align*}
    ...
    \end{align*}
    and thus combining the above with Proposition~\ref{proposition:composition_formula},
    we obtain
    \begin{align*}
...
\begin{proof}
    ...
    \end{align*}
    and thus combining the above with Proposition~\ref{proposition:adjoint_formula},
    we obtain
    \begin{align*}
        \SymbolSemiNorm {m - (\rho - \delta)(N + 1)} {\rho, \delta} P {\tau_N}
        = \liminf_{\epsilon \to 0}
...
\begin{align*}
    ...
    \quad h \neq e_\Group,
\end{align*}
which is problematic because $-\dim \Group$ is a \emph{critical exponent} due to the Sobolev inequality.
...
\begin{remark}
    ...
    but the ideas presented are based upon V\'eronique Fischer's privately communicated proof in the compact and graded setting.
\end{remark}

The following lemma is obtained by a very simple modification of~\cite[Lemma A.6]{Fischer2015},
which relies mainly on classical heat kernel estimates valid for Lie groups with polynomial growth of the volume,
...
Naturally, this implies that $T \in \OperatorClass m {1, 0}$ is bounded between $\SobolevOrder p s$ and $\SobolevOrder p {s - m}$ for each $s \in \R$.
Since $\BesselPotential m \in \OperatorClass m {1, 0}$,
it follows that these $\Lebesgue p \Group$ estimates are sharp.
...
\begin{align*}
    E_\lambda(\Lambda)
    \defeq
    \Id {\bigoplus_{\JapaneseBracket \CompactGroup \tau + \abs \lambda^2 \geq \Lambda} \Span \{\tau_{ij} : i, j = 1, \dots, \dimRep \tau\}}
\end{align*}
when $\lambda \in \VectorSpace$ and $\Lambda \geq 0$.
...
\label{theorem:construction_of_parametrices}
    ...
    There exists an operator $B \in \OperatorClass {-m} {\rho, \delta}$ which satisfies
    \begin{align*}
