...
\begin{proof}
    ...
    \begin{align}
        &\LeftDifferentialOperatorFirstOrder Y_l \sigma(x, k; l \lambda) \notag\\
        &\quad = \sum_{j = 1}^{\dim \VectorSpace} \eval{\D*{1}{s}}{s = 0} \sigma(x, k; l \lambda + s u e_j) \D*{t}<t = 0> \ip {l \exp_\CompactGroup (t Y) \lambda} {u e_j}.
    ...
    \begin{align}
        \LeftDifferentialOperatorFirstOrder Y_l \tilde \sigma(x, k; l \lambda)
        &= \sum_{j = 1}^{\dim \VectorSpace} \eval{\D*{1}{s}}{s = 0} \tilde \sigma(x, k; l \lambda + s e_j) \D*{t}<t = 0> \ip {l \exp_\CompactGroup (t Y) \lambda} {e_j}.
...
\begin{proof}
    ...
    Since the operator inside $\InverseRotation \dummy$ on the second line is in $\SymbolClass {-\frac {\dim \CompactGroup} 2 (1 - \rho) } {\rho, \delta'}$,
    ...
    \begin{align*}
        ...
        \norm [\Lin {\Lebesgue 2 \CompactGroup}] {%
            \Rep[\GroupDirect] \lambda \BesselPotential{-m' + \rho \abs \alpha  - \delta' \abs \beta + \gamma} \DifferenceOperator [\GroupDirect] {\tilde q} X^\beta \tilde \sigma(x, k; \lambda) \Rep [\GroupDirect] \lambda \BesselPotential{-\gamma}
        }
    \end{align*}
    is finite,
