...
\begin{example}
    ...
    \begin{align*}
        \Lambda \defeq \{r \e^{\i \theta} : r \geq 0 \text{ and } \frac {2 \pi} 3 \leq \theta \leq \frac {4 \pi} 3 \} \cup \{z \in \C \abs z \leq \frac 1 2  \}
    \end{align*}
\end{example}
...
\label{theorem:generalised_Littlewood-Paley_decomposition}
    ...
    we let
    \begin{align*}
        \eta_\epsilon(\lambda) \defeq \e^{-\epsilon^2 \Rep \lambda \BesselPotentialSquared 2}
    \end{align*}
    ...
    In particular, for each $m > 0$, $\eta_\epsion$ converges to $\Id {\Lebesgue 2 \CompactGroup}$ in $\SymbolClass m {\rho, \delta}$.
...
\begin{proof}
    ...
    We also let $\Gamma$ be the boundary of $\Lambda \cup \{z \in \C : \abs z \leq \delta\}$ with $0 < \delta \leq 1 / 2$.

    Let $m < 0$ and write $F_\epsilon(z) = {\e^{-(\epsilon z)^2}}$.
    ...
    we let
    \begin{align*}
        G_\epsilon(z) \defeq (\epsilon z)^{-\frac m 2} (1 - \e^{-(\epsilon z)^2})
    \end{align*}
    and observe that
    since $G_1$ vanishes at order $2$ at the origin,
    ...
    Defining
    \begin{align*}
        G_\epsilon(z) = (\epsilon z)^{-1} (1 - \e^{-(\epsilon z)^2})
    \end{align*}
    it is clear that
    \begin{align*}
...
\label{proposition:composition_formula}
    Assume that $1 \geq \rho > \delta \geq 0$.
    For each $m_1, m_2 \in \R$, and each $M, N \in \N$,
    there exists $C > 0$ and $M' \in \N$ such that
    \begin{align*}
        &\SymbolSemiNorm {m_1 + m_2 - N (\rho - \delta), R} {\rho, \delta} M {%
        (\sigma_1 \circ \sigma_2)(g, \lambda)
        - \sum_{\abs \alpha \leq N} \DifferenceOperatorOrder \alpha \sigma_1(g, \lambda) \LeftDifferentialOperator \alpha \sigma_2(g, \lambda)
        }\\
        &\qquad \leq C
        \SymbolSemiNorm {m_1, R} {\rho, \delta} {M'} {\sigma_1}
        \SymbolSemiNorm {m_2} {\rho, \delta} {M'} {\sigma_2}
    \end{align*}
...
\begin{lemma}
    Assume that $1 \geq \rho > \delta \geq 0$.
    For each $\beta_0$ and each $M \in \N$,
    there exists $C \geq 0$ and an $N \in \N$ such that for any $\sigma \in \SmoothingSymbols$,
    \begin{align*}
        &\norm [\Lin {\Lebesgue 2 \Group}] {%
        ...
        &\quad \leq C \SymbolSemiNorm m {\rho, \delta} N \sigma
    \end{align*}
...
\begin{proof}
    ...
    \begin{align*}
        ...
        &= \sum_{\abs {\beta_1} + \abs {\beta_2} \leq 2M} \int_\Group \TaylorRemainder {\RightDifferentialOperator [g_2 = g] {\beta_2} \LeftDifferentialOperator [g_1] {\beta_1} \LeftDifferentialOperator {\beta_0} \kappa_{g g_1}^{(*)}(h)} {g_1 = e_\Group} {N - \abs {\beta_1}} (h^{-1}) \adj{\Rep \lambda (h)} \dd h.
    \end{align*}
    ...
    \begin{align*}
        ...
        \norm [\Lin {\Lebesgue 2 \CompactGroup}] {%
            \TaylorRemainder {\RightDifferentialOperator [g_2 = g] {\beta_2} \LeftDifferentialOperator [g_1] {\beta_1} \LeftDifferentialOperator {\beta_0} \kappa_{g g_1}^{(*)}(h)} {g_1 = e_\Group} {N - \abs {\beta_1}} (h^{-1})
        }
        \dd h.
    \end{align*}
...
\label{proposition:adjoint_formula}
    Assume that $1 \geq \rho > \delta \geq 0$.
    For each $m \in \R$ and each $M, N \in \N$,
    there exists $C \geq 0$ and $M' \in \N$ such that
    \begin{align*}
        \SymbolSemiNorm {m - N (\rho - \delta), R} {\rho, \delta} M {%
        \sigma^{(*)}(g, \lambda)
        - \sum_{\abs \alpha \leq N} \DifferenceOperatorOrder \alpha \LeftDifferentialOperator \alpha \adj \sigma(g, \lambda)
        } \leq
        \SymbolSemiNorm m {\rho, \delta} {M'} \sigma
    \end{align*}
...
We are now able to finish the proof of the \emph{adjunction} and \emph{composition} formulas.
...
\begin{proof}
    Fix $\epsilon \in (0, 1]$.
    and denote by $\sigma_{1, \epsilon}, \sigma_{2, \epsilon} \in \SmoothingSymbols$ the symbols given by Proposition~\ref{proposition:approximation_of_symbols}.
    By Proposition~\ref{proposition:composition_formula},
    for each $M, N \in \N$,
    we can find $M' \in \N$ such that
    \begin{align*}
        &\SymbolSemiNorm {m_1 + m_2 + 2 \eta - N (\rho - \delta), R} {\rho, \delta} M {%
        (\sigma_{1, \epsilon} \circ \sigma_{2, \epsilon})(g, \lambda)
        - \sum_{\abs \alpha \leq N} \DifferenceOperatorOrder \alpha \sigma_{1, \epsilon}(g, \lambda) \LeftDifferentialOperator \alpha \sigma_{2, \epsilon}(g, \lambda)
        }\\
        &\qquad \leq C
        \SymbolSemiNorm {m_1 + \eta, R} {\rho, \delta} {M'} {\sigma_{1, \epsilon}}
        \SymbolSemiNorm {m_2 + \eta} {\rho, \delta} {M'} {\sigma_{2, \epsilon}}\\
        &\qquad \leq C \epsilon^{2 \eta}
        \SymbolSemiNorm {m_1, R} {\rho, \delta} {M'} {\sigma_1}
        \SymbolSemiNorm {m_2} {\rho, \delta} {M'} {\sigma_2}\\
    \end{align*}
\end{proof}
