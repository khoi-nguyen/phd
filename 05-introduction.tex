...
while their Euclidean counterparts make sense without restrictions on $\rho$ and $\delta$ other than $\rho \geq \delta$.
Secondly,
...
\begin{description}
    ...
    \item[Approximation by smoothing symbols]
        ...
        In particular,
        this allows us to show the composition and adjunction formulae only
        for smoothing symbols before taking the limit $\epsilon \to 0$.
\end{description}
...
\begin{align*}
    ...
    \frac 1 {\i \turn} \int_\Gamma \e^{-\epsilon z} \left(\xi(\Id {} - \Laplacian) - z \Id {}\right)^{-1} \dd z,
    \quad \xi \in \dualGroup \Group
\end{align*}
with an appropriate contour $\Gamma \subset \C$.
We also show the symbolic estimates on $\eta_\epsilon$ and $\Id {} - \eta_\epsilon$
...
(provided that we have good properties for difference operators),
...
\begin{itemize}
    ...
    \item
        The kernel estimates in the case $\rho = 1$,
...
We then conclude with the classical application of pseudo-differential calculus to the construction of parametrices.
