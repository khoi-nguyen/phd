...
As the composition and the adjunction of pseudo-differential operators is done at the level of the kernel,
it is crucial for various technical reasons to understand how kernels of pseudo-differential operators behave.
...
The difficulty lies in estimating the second term.
To this end,
we observe that
\begin{align*}
    &\abs {q^\alpha(h) (\kappa_g(h) - \conv {\kappa_g} {p_\epsilon}(h))}\\
    &\quad \leq
    \sum_{\alpha_1, \alpha_2}
    \abs {\conv {q^{\alpha_1}(h) \kappa_g} {q^{\alpha_2}(h) (\delta_{e_\Group} - p_\epsilon)(h)}}\\
    &\quad \leq
    \int_{\dualGroup \Group}
    \tr\left|
    \DifferenceOperatorOrder {\alpha_1} \sigma(g, \xi)
    \DifferenceOperatorOrder {\alpha_2} (\Id{} - \eta_\epsilon(\xi))
    \right| \dd \mu_{\dualGroup \Group}(\xi)
\end{align*}
where the last line was obtained by the Fourier inverse formula.

Using that $\sigma \in \SymbolClass m {\rho, \delta}$,
we obtain
\begin{align*}
    &\abs {q^\alpha(h) (\kappa_g(h) - \conv {\kappa_g} {p_\epsilon}(h))}\\
    &\quad \leq
    \int_{\dualGroup \Group}
    \tr\left|
    \xi \BesselPotential {m - \rho \abs \alpha}
    \DifferenceOperatorOrder {\alpha_2} (\Id{} - \eta_\epsilon(\xi))
    \xi \BesselPotential {\gamma - \gamma}
    \right| \dd \mu_{\dualGroup \Group}(\xi)\\
    &\quad \leq C \epsilon^{\frac {-m + \rho \abs \alpha - \gamma} \nu}
    \int_{\dualGroup \Group} \tr(\xi \BesselPotential {-\gamma}) \dd \mu_{\dualGroup \Group}(\xi)
\end{align*}
provided that
\begin{align*}
    \begin{cases}
        m - \rho \abs \alpha + \gamma < 0\\
        \gamma > N,
    \end{cases}
\end{align*}
where $N \in \N$ is the Sobolev embedding exponent.

The proof follows by choosing $\rho$ and $\gamma$ so that
\begin{align*}
    \epsilon^{\frac {-m + \rho \abs \alpha - \gamma} \nu}
    = \epsilon^{-\frac {m + N} \nu}
\end{align*}
and remembering that $\epsilon^\nu$ equals the distance between $h$ and the origin $e_\Group$.
