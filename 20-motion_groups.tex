...
Since $\LieAlgebraCompactgroup \subset \GeneralLinear \VectorSpace$,
it follows that $\LieAlgebraCompactgroup$ is also an algebra acting on $\VectorSpace$.
...
\begin{definition}[Lie algebra of $\Group$]
    ...
    we let
    \begin{align*}
        (X_1, Y_1) (X_2, Y_2) = (X_1 + Y_1 X_2, Y_1, Y_2),
    \end{align*}
    which gives $\LieAlgebra$ an \emph{algebraic structure}.

    Therefore, if $X, Y \in \LieAlgebra$,
    we can define its \emph{Lie bracket} via
    \begin{align*}
