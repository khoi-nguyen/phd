...
\begin{lemma}
    ...
    \end{align*}
    In particular,
    if a sequence $(f_j)_{j \in \N} \subset \Sobolev s$ converges to $f$ in $\Sobolev s$, then
    \begin{align*}
        \lim_{j \to \infty} \int_\VectorSpace \tr(\Fourier {f_j}(\lambda)) \dd \lambda
        = \int_\VectorSpace \tr(\Fourier f(\lambda)) \dd \lambda.
    \end{align*}
\end{lemma}
...
\end{proposition}
\begin{proof}
    % TODO: Put the h bit in a separate claim or lemma
    Suppose that $h \in \Sobolev s$.
    By definition, the distribution
    \begin{align*}
        h_s \defeq \BesselPotential s h
    \end{align*}
    belongs to $\Lebesgue 2 \Group$.
    By the Cauchy-Schwartz inequality for Schatten classes,
    we have
    \begin{align}
        \norm [\SchattenClasses 1 {\Lebesgue 2 \CompactGroup}] {\Fourier h(\lambda)}
        \leq
        \norm [\SchattenClasses 2 {\Lebesgue 2 \CompactGroup}] {\Rep \lambda \BesselPotential {-s}}
        \norm [\SchattenClasses 2 {\Lebesgue 2 \CompactGroup}] {\Fourier h_s(\lambda)}.
    ...
    \begin{align*}
        ...
        < \infty,
    \end{align*}
    i.e.\ $\Fourier h(\lambda)$ is trace class for almost every $\lambda \in \VectorSpace$.
    ...
    The first claim follows from applying the above to $h \defeq f$,
    so we only need to check the actual inverse formula.

    To this end,
    select a sequence ${(f_j)}_{j \in \N} \subset \Schwartz \Group$ such that
    \begin{align*}
        \lim_{j \to \infty} \norm [\Sobolev s] {f_j - f} = 0.
    \end{align*}

    By the Sobolev embeddings (Proposition~\ref{proposition:Sobolev_embedding}),
    we also have \emph{uniform convergence}, hence
    \begin{align*}
        f(g)
        &= \lim_{j \to \infty} f_j(g)\\
        &= \lim_{j \to \infty} \int_\VectorSpace \tr\left(\Rep \lambda(g) \Fourier f_j(\lambda)\right) \dd \lambda,
    \end{align*}
    where the last line was obtained via the classical Fourier inverse formula (Proposition \ref{proposition:inverse_Fourier_Transform}).

    Using~\eqref{eq:Sobolev_inclusion_into_fields_with_integrable_S1_norm} with $h \defeq f_j - f$,
    we obtain that in fact
    \begin{align*}
        f(g)
