...
Moreover,
the representations of the motion group seem formally very close to those of $\GroupDirect$, provided that they are grouped on both sides.
...
\begin{align*}
    g_{x, k} \circ g_{y, l} = g_{x + k y, k l},
\end{align*}
...
\index{Euclidean motion group}
    For each $n \in \N$, let
    \begin{align*}
        \MotionGroup{n} \defeq \{g \in \AffineTransformations{\R^n} : g(x) = Ax + y,\ A \in \SpecialOrthogonalGroup n,\ x, y \in \R^n\}.
    \end{align*}
...
\begin{example}
    ...
    Consider the group
    \begin{align*}
        \{g \in \AffineTransformations{\C^n} :  g(x) = Ux + y,\ U \in \SpecialUnitaryGroup n, \ y \in \C^n \}
    \end{align*}
    where the law is the composition of functions.
    Arguing as in Example~\ref{example:Euclidean_motion_groups},
...
\begin{proof}
    ...
    \end{align*}
    where $\dd k$ is the normalized Haar measure on $\CompactGroup$.
    ...
    Now, if $x \in \VectorSpace$ are such that $\ip{x}{x} = 0$,
...
\label{lemma:Haar_measure}
    ...
    then the product measure $\dd x \dd k$ is a Haar measure on $\Group = \VectorSpace \ltimes \CompactGroup$,
    which is both left and right-invariant.
\end{lemma}
\begin{proof}
    Let $(x, k) \in \Group$ and $f \in \Lebesgue 1 \Group$.
    \begin{align*}
    ...
    As the Lebesgue measure is invariant under $\SpecialOrthogonalGroup{\VectorSpace}$ and under translations,
    ...
    \begin{enumerate}
        \item This follows easily from the change of variables formula,
            \begin{align*}
                ...
                = \int_A 1 \dd x,
            \end{align*}
            where we use the fact that $\det k = 1$ since $k \in \SpecialOrthogonalGroup\VectorSpace$.
...
The difference between the semi-direct structure and its direct counterpart appears naturally when we study the \emph{Lie algebra}.
...
We now describe explicitly the Lie algebra of Euclidean motion groups.
...
\begin{example}[Euclidean motion groups]
    ...
    \begin{align*}
        E_{ij} e_k = \Kronecker i k e_j - \Kronecker j k e_i
    \end{align*}
...
Let us compute the expression of a left-invariant derivative arising from $\VectorSpace \subset \LieAlgebra$.
...
The following definition is a very important example of left-invariant differential operator.
    ...
    \begin{itemize}
        \item
            ...
            \begin{align*}
                ...
                = \ip {\grad \phi(x, k)} X,
            \end{align*}
            where $(x, k) \in \Group$ and $\phi \in \SmoothFunctions \Group$.
        \item
            ...
            \end{align*}
            where $(x, k) \in \Group$, $\phi \in \SmoothFunctions \Group$,
            and $\RightDifferentialOperatorFirstOrder Y_k$ is the right-invariant vector field associated with $Y$ on the manifold $\CompactGroup$ acting exclusively on the variable $k \in \CompactGroup$.
    \end{itemize}
\end{proposition}
\begin{proof}
...
\begin{corollary}
    ...
    \begin{itemize}
        ...
        \item
            ...
            \end{align*}
            where $\RightDifferentialOperatorFirstOrder Y_k$ is the right-invariant vector field associated with $Y$ on the manifold $\CompactGroup$ acting exclusively on the variable $k \in \CompactGroup$.
    \end{itemize}
\end{corollary}
...
where the author observes that the Plancherel formula takes a more elegant form if we group the unitary representations appropriately.
His subsequent papers (e.g.\ \cite{Kumahara76}) work exclusively with those grouped representations as they are technically more convenient.
...
\begin{itemize}
    ...
    \item
        ...
        Again,
        we choose to work with them nevertheless
...
because they characterize the action of left- (respectively right-) invariant differential operators on the Fourier Transform side.
As such,
...
\begin{definition}[Schwartz space]
    ...
    and give it a structure of a \emph{Fr\'echet space}.
\end{definition}
...
\index{motion group!Fourier transform}
    Let $f \in \Lebesgue{1}{\Group}$ and $\lambda \in {\VectorSpace}$.
    ...
    Moreover, the map
    \begin{align*}
        \Fourier{f} : {\VectorSpace} \to \End(\Lebesgue{2}{\CompactGroup}) :
...
\begin{proof}
    ...
    Recognizing the Fourier Transform on $\VectorSpace$ in the above, we obtain
    \begin{align*}
        \Fourier f(\lambda) F(u)
        &=
        \int_\CompactGroup
...
\begin{proof}
    ...
    \end{align*}
    where the last line is obtained by substituing $k$ for $u k^{-1}$.

    Recognizing a Fourier transform on $\CompactGroup$ with ``grouped representations''
...
\index{motion group!Fourier transform!inverse formula}
    ...
    \begin{align*}
        \phi(g)
        = \int_{\VectorSpace}
        \tr \left( \Rep{\lambda}(g) \Fourier \phi(\lambda) \right) \dd \lambda.
    \end{align*}
\end{proposition}
\begin{proof}
    ...
    \end{align*}
    where the last line is obtained by a change of variables after permuting the integrals.

    Recognizing an inverse Fourier Transform on the right-hand side of the above, we obtain
    \begin{align*}
        \int_\VectorSpace \tr(\Fourier \phi(\lambda)) \dd \lambda
        = \phi(0, e),
    \end{align*}
    ...
    \end{align*}
    where the last equality is obained by Proposition~\ref{proposition:elementary_properties_of_the_Fourier_transform}.
\end{proof}

\subsubsection{The direct product case}

In view of Subsubsection~\ref{subsubsection:representations_on_the_direct_product},
let us compare the Plancherel formulae.
...
Although Subsection~\ref{subsection:Fourier_transform_of_distributions} already treated the problem of generalizing the Fourier Transform for the purposes of pseudo-differential calculus,
let us mention some results that have been obtained explicitly for the motion group.
...
\begin{definition}[Image of the Schwartz space]
    ...
    the set of all functions $F \in \SmoothFunctions{\VectorSpace, \Lin{\Lebesgue{2}\CompactGroup}}$ satisfying the following properties.
    \begin{enumerate}
...
As we wish to define pseudo-differential operators with non-integer orders,
...
\begin{proof}
    ...
    \begin{itemize}
        ...
        \item
            ...
            so that a density argument yields the existence of a unique continuous extension of
            \begin{align*}
                \LeftDifferentialOperator \alpha : \Sobolev s \to \Sobolev {s - \abs \alpha}.
            \end{align*}
...
\begin{proof}
    ...
    \end{align*}
    However, \cite[Lemma 6.3]{Fischer2015} implies that the above converges if and only if
    \begin{align*}
...
\begin{proof}
    ...
    By the Cauchy-Schwarz inequality for Schatten classes,
    we have
    \begin{align}
    ...
    \end{align}
    where the last line was obtained via the Cauchy-Schwarz inequality on $\Lebesgue 2 \VectorSpace$
...
We are now ready to prove the following generalization of Proposition~\ref{proposition:inverse_Fourier_Transform}.
...
To insist upon the fact
that the basis is adapted to our family of smooth functions,
we shall write
\begin{align*}
...
By using local charts on $\CompactGroup$ and then subsequently extending onto all of $\CompactGroup$ via a partition of unity,
