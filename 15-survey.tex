...
\begin{definition}[Symbol classes on $\CompactGroup$]
    ...
    \end{align*}
    is a \emph{symbol of order $m$} and of \emph{type $(\rho, \delta)$} if
    \begin{align*}
        \sup_{k \in \CompactGroup}
        \norm [\Lin {\Lebesgue 2 \CompactGroup}] {%
            \Rep [\CompactGroup] {} \BesselPotential [\CompactGroup] {-m}
            \LeftDifferentialOperator \beta \DifferenceOperatorOrder [\CompactGroup] \alpha \sigma(k)
        } < \infty
    \end{align*}
    for every $\beta \in \N^{\dim \VectorSpace}$ and every $\alpha$.
